"""Ejecucion y despacho de BIENES en el avance de bolsa.

Revision ID: a7e5f8c1d2b9
Revises: f3c9d1a2b4e6
Create Date: 2026-07-31

Hallazgos (backend/scripts/diagnostico_sesion6, medidos contra SIGA 2026):

1. Las 611 ordenes de bienes del kanban tenian ejecucion=0 y despacho=0. La
   conformidad (SIG_MOVIM_CONFOR_SERVICIO) es SOLO de servicios, y la pecosa
   se buscaba en los items del pedido de compra ('2'), que nunca la tienen
   (vive en los pedidos de atencion '1').
2. La recepcion de bienes SI tiene llave dura: la entrada de almacen
   (movimientos_almacen, TIPO_MOVIMTO='I') referencia NRO_ORDEN en el 100%
   de sus filas y cubre 572/632 ordenes de bienes (90.5%).
3. El despacho (pecosa) es recuperable via puente DECLARATIVO: el pedido de
   atencion ('1') nombra la O/C en su motivo ("ATENCION DE PEDIDO A LA O/C
   N°570") en 531/576 casos, y sus items cruzan con la salida de almacen
   ('S') al 99.4%.
4. El devengado de bienes NO existe en SIGA local (SIG_EXP_SIGA_DOCU solo
   trae operaciones CP/N; ESTADO_SIAF='2' esta en el 99.7% de las ordenes y
   no discrimina). Queda sin fecha: el devengado autoritativo es MEF.

Cambios:
- v_bolsa_avance: fecha_ejecucion ahora cubre bienes (entrada de almacen por
  orden) ademas de servicios (conformidad); nueva columna fecha_despacho
  (primera salida de almacen de un pedido '1' que nombra una O/C de la bolsa).
- v_pipeline_pedido: expone bolsa_fecha_despacho.
"""
...
revision: str = "a7e5f8c1d2b9"
down_revision: str | None = "f3c9d1a2b4e6"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


_V_BOLSA_AVANCE_NUEVA = r"""
...
WITH ccmn_bolsa AS (
    ...
    SELECT b.ano_eje, b.sec_ejec, b.tipo_bien, b.sec_cua_mod_sal,
           ...
           -- Ejecucion: conformidad (servicios) o entrada de almacen (bienes).
    ...
    LEFT JOIN LATERAL (
        SELECT MIN(cf.fecha_movimto) AS fecha_confor,
               ...
               -- en estado devengado (§02.2, caso 232/S). Para bienes no hay
               -- fecha de devengado en SIGA local: queda NULL (fuente: MEF).
    ...
    LEFT JOIN LATERAL (
        -- Recepcion de bienes: entrada de almacen que referencia la O/C.
        -- Llave dura, 100% de las entradas traen NRO_ORDEN (diag. sesion6/05).
    ...
    LEFT JOIN LATERAL (
        -- Despacho (pecosa) de bienes: puente DECLARATIVO. El pedido de
        -- atencion ('1') nombra la O/C en su motivo; sus items llevan la
        -- pecosa y esta cruza con la salida de almacen (diag. sesion6/06).
...
# Identica a la de f3c9d1a2b4e6 + bolsa_fecha_despacho.
...
def upgrade() -> None:
    ...
    op.execute(_V_BOLSA_AVANCE_NUEVA)
    op.execute(_V_PIPELINE_PEDIDO_NUEVA)
    for sql in _INDICES:
        op.execute(sql)


# Definiciones previas (e2b3c4d5f6a8 para la vista, f3c9d1a2b4e6 para la MV).
_V_BOLSA_AVANCE_PREVIA = """
CREATE VIEW siga.v_bolsa_avance AS
WITH ccmn_bolsa AS (
    SELECT b.ano_eje, b.sec_ejec, b.tipo_bien, b.sec_cua_mod_sal,
           b.nro_consolid,
...
_V_PIPELINE_PEDIDO_PREVIA = _V_PIPELINE_PEDIDO_NUEVA.replace(
    "    ba.fecha_despacho                                     AS bolsa_fecha_despacho,\n",
    "",
)
